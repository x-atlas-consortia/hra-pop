...
function getCellCountsSimilarity(cellsA, cellsB) {
  ...
  if (sharedKey) {
    const keys = [...keySet];
    const valuesA = new Float32Array(keys.map((key) => cellsA[key] ?? 0));
    const valuesB = new Float32Array(keys.map((key) => cellsB[key] ?? 0));
    return 1 - cosine(valuesA, valuesB);
  } else {
    return 0;
  }
}
